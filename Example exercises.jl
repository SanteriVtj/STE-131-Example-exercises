...
using JuMP, HiGHS, Random, Distributions, GLMakie, DataFrames, PlutoUI, LaTeXStrings
...
You get the following data that is produced by an imaginary forest conservation auction. The data set, denoted by `data`, consists of two variables ``\text{bids}\in\mathbb{R}^N`` measured in Euros and ``\text{EB}\in\mathbb{R}^N`` short for environmental benefit giving us a way to measure the benefit of conserving particular plot of forest. The objective is to choose an optimal allocation of forest land to purchase from the sellers as an organizer of the auction to maximize the environmentala benefit under a resource constraint such that the total available budget is ``B=500``. Spesifically:
...
> Compute optimal allocation by choosing ``m`` cheapest bids so that ``\sum_{i=1}^mb_i\leq B``.

### c)

> Compute optimal allocation by choosing ``m`` highest environmental benefit bids so that ``\sum_{i=1}^mb_i\leq B``.

### d)

> Use each of the allocations to compute the €/EB ratio for each meachnism and compare the allocations graphically. Which of the mechanisms achieves highest environmental benefit for lowest price?

By changing the sliders below you can change the data generating process and see how thet solution changes as the data changes.
"""
...
Markdown.parse(
	"""To solve the optimization problem posed at the beginning, one needs to consider what is the objective and what kind of constraints does the setup of the problem introduce. The given objective is to maximize the amount of environmental benefit acquired by buying some not predetermined number of forest plots with the given budget ``B=$(B)``. Lets represent the choice of forest plots by a vector ``x\\in\\{0,1\\}^{$(N)}``, so that the each element of the vector is either ``0`` or ``1`` and tells whether the auctioneer intends to buy the plot from the auction.
	
	Then the optimization problem can be formulated as a sum ``\\sum_{i=1}^{$(N)}x_i\\text{EB}_i=\\text{EB}^\\top x`` and this needs to satisfy the budget constraint, which can be computed similarly as ``\\text{bids}^\\top x\\leq B=$(B)``. Putting all of this together yields:
	
	```math
	\\begin{aligned}
		\\text{max}&_{x}\\; \\text{EB}^\\top x \\\\
		&\\text{s.t.} \\\\
		&\\quad \\text{bids}^\\top x\\leq $(B) \\\\
		&\\quad x_i\\in\\{0,1\\}:\\; \\forall i\\in\\{1,2,\\ldots,$N\\}
	\\end{aligned}
	```
	This can now be computed using `JuMP.jl` modeling language and `HiGHS.jl` solver as:"""
)
...
The allocations for parts **b** and **c** can be solved by simply choosing the ``m`` lowst bids and correspondingly ``m`` highest environmental benefits. In the following code, the data frame is first sorted by these variables and then ``m`` bids are accepted the until the budget constraint gets violated.
"""
...
The cost of each allocation can be computed by multiplying the bids by the decision vector ``\text{bids}^\top x``. For the optimized allocation this gives a price of $(milp_cost)€. With the same allocation the environmental benefit acquired can be computed as ``\text{EB}^\top x`` and the benefit from this allocation is $(milp_eb).

Similarly the €/EB ratio can be computed as the ratio between the two quantities as:
```math
	\frac{\text{bids}^\top x}{\text{EB}^\top x}.
```
For the optimization this yields a value of $(milp_ratio). All of the values are still summarized below in a table.
...
manifest_format = "2.1"
project_hash = "0c30e3ea39e58f5e43af5c3183f901638e5b954b"
...
git-tree-sha1 = "39360b4b61003a316ba98ba5a1c6521fef7e3e35"
registries = "General"
...
git-tree-sha1 = "45c40ea8a53abfc8d14408a841a0c2be27b652af"
registries = "General"
