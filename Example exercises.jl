...
# ╔═╡ 656a7f78-add5-11f1-846e-dd701d12f671
using JuMP, HiGHS, Random, Distributions, CairoMakie, DataFrames, PlutoUI, LaTeXStrings

# ╔═╡ 8e43b852-03c1-498d-b764-32805b5829d1
Random.seed!(123);

# ╔═╡ 9a06d4ef-bbb7-4778-892d-a438dc85370d
TableOfContents()

# ╔═╡ 3d3b9c65-c483-4443-b03a-71dbd0de5666
md"""
# Example exercise: choosing an optimal allocation from an environmental auction

## Exercise 1

You are given the following dataset, generated from an imaginary forest conservation auction. The dataset, denoted by `data`, consists of two variables: ``\text{bids}\in\mathbb{R}^N``, measured in euros, and ``\text{EB}\in\mathbb{R}^N``, where EB stands for environmental benefit and provides a measure of the benefit obtained from conserving a particular forest plot.

As the organizer of the auction, your objective is to choose an optimal allocation of forest plots to purchase from the sellers in order to maximize the total environmental benefit subject to a budget constraint. The total available budget is ``B=500``.

Specifically:

### a)

> Formulate the problem as a mixed-integer linear program and compute the optimal allocation.

### b)

> Compute an allocation by choosing the ``m`` cheapest bids such that ``\sum_{i=1}^m b_i\leq B``.

### c)

> Compute an allocation by choosing the ``m`` bids with the highest environmental benefits such that ``\sum_{i=1}^m b_i\leq B``.

### d)

> For each allocation, compute the €/EB ratio and compare the allocations graphically. Which of the mechanisms achieves the highest environmental benefit at the lowest cost?

By changing the sliders below, you can modify the data-generating process and observe how the solution changes as the data change.
"""
...
# ╔═╡ 486bab53-29a6-41ae-bee7-be658730d982
@bindname σ_bid PlutoUI.Slider(1:.5:50; default=10, show_value=true)

# ╔═╡ 588aee00-46df-4386-a058-6c2db22a4d6c
begin
	# Compute the covariance matrix from the standard deviations and correlation
	Σ = [
		σ_bid^2 		ρ*σ_bid*σ_EB; 
		ρ*σ_bid*σ_EB 	σ_EB^2
	]
...
Lets first see the data by plotting the bids against the environmental benefit.
"""

# ╔═╡ 49276772-12fa-42a9-89db-e9ead2e21c2a
begin
	f = Figure()
	ax = Axis(
		f[1,1],
		xlabel="Bids",
		ylabel="Environmental benefit",
		title="Auction data"
	)
	scatter!(ax, data[!,:bids], data[!,:EB])
	f
end

# ╔═╡ 8db29588-9d16-43af-a78d-ca584cc42d99
Markdown.parse(
"""To solve the optimization problem posed above, we first need to identify the objective and the constraints implied by the problem setup. The objective is to maximize the total environmental benefit obtained by purchasing a subset of the forest plots subject to the available budget `B=$(B)`.

Let the selection of forest plots be represented by a vector ``x\\in\\{0,1\\}^{$(N)}``, where each element of the vector is either ``0`` or ``1`` and indicates whether the auctioneer chooses to purchase the corresponding plot.

The objective can then be written as ``\\sum_{i=1}^{$(N)}x_i\\text{EB}_i=\\text{EB}^\\top x``. This must satisfy the budget constraint, which can similarly be written as ``\\text{bids}^\\top x\\leq B=$(B)``.

Putting everything together, the optimization problem becomes:

```math
\\begin{aligned}
	\\max_{x}\\quad & \\text{EB}^\\top x \\\\
	\\text{s.t.}\\quad
	& \\text{bids}^\\top x\\leq $(B) \\\\
...
This problem can now be solved using the `JuMP.jl` modeling language and the `HiGHS.jl` solver as follows:"""
)


# ╔═╡ 77d013f4-9113-42e6-8140-fe4f9df0225f
function solve_allocation(X, B)
	# Number of bids
	N = size(X,1)
	# Create a model
	...
	@constraint(model, X[!,:bids]'*x ≤ B)

	# Optimize the model defined above
	optimize!(model)
...
# ╔═╡ 1f9cc7e4-3206-4aa4-a9ca-02180f1e98c4
begin
	# Solve the optimal allocation
...
begin
	...
	allocation_ax = Axis(
		...
		xlabel="Bids",
		ylabel="Environmental benefit",
		title="MILP allocation"
	)
	scatter!(
		allocation_ax, 
		data[data[!,:milp_allocation], :bids],		
		data[data[!,:milp_allocation], :EB],
		label="Chosen",
		color=:green
	)
	scatter!(
...
# ╔═╡ 4f4408b5-0c87-428d-8426-e7ca4a0711a8
md"""

### b & c)

The allocations for parts **b** and **c** can be obtained by selecting the `m` lowest bids and, correspondingly, the `m` highest environmental benefits. In the following code, the data frame is first sorted according to the relevant variable, after which bids are accepted in order until accepting the next bid would violate the budget constraint.
"""


# ╔═╡ aa1d0a98-f410-40ec-aa7d-1759a0bc3c39
begin
	# Compute allocation by choosing lowest bids
	sort!(data, [:bids])
	data[!, "bid_allocation"] = cumsum(data[!,:bids]) .≤ B
...
The cost of each allocation can be computed by multiplying the bid vector by the decision vector, ``\text{bids}^\top x``. For the optimized allocation, this gives a total cost of $(milp_cost)€. Using the same allocation, the total environmental benefit can be computed as ``\text{EB}^\top x``, yielding an environmental benefit of $(milp_eb).

Similarly, the €/EB ratio can be computed as the ratio between these two quantities:
```math
	\frac{\text{bids}^\top x}{\text{EB}^\top x}.
```

For the optimized allocation, this yields a value of $(milp_ratio). All results are summarized in the table below.

| Decision rule | Cost (€) | EB | €/EB |
| :--- | :---: | :---: | :---: |
| MILP | $(milp_cost) | $(milp_eb) | $(milp_ratio) |
| Lowest cost | $(cost_cost) | $(cost_eb) | $(cost_ratio) |
| Highest EB | $(eb_cost) | $(eb_eb) | $(eb_ratio) |

To better understand how the decision rules differ, we can also visualize the selected bids. The plot below shows that the optimization takes both cost and environmental benefit into account, whereas the simpler decision rules, by definition, consider only one of these variables.

The optimized allocation often produces a more efficient outcome in terms of €/EB, meaning that the auctioneer obtains more environmental benefit for each euro spent. In some cases, selecting the bids with the highest environmental benefits first can result in a lower €/EB ratio. However, because this rule may select relatively expensive bids early on, it can leave part of the budget unused and therefore achieve a lower total environmental benefit.
"""

# ╔═╡ 58232494-4eb4-48c8-9070-e22004f6dde5
# begin
# 	println("With MILP allocation, cost of $(round(data[!,:milp_allocation]'*data[!,:bids]; digits=2)) yields $(round(data[!,:milp_allocation]'*data[!,:EB]; digits=2)) environmental benefit.\n")
	
# 	println("With cost allocation, cost of $(round(data[!,:bid_allocation]'*data[!,:bids]; digits=2)) yields $(round(data[!,:bid_allocation]'*data[!,:EB]; digits=2)) environmental benefit.\n")
	
# 	println("With environmental benefit allocation, cost of $(round(data[!,:EB_allocation]'*data[!,:bids]; digits=2)) yields $(round(data[!,:EB_allocation]'*data[!,:EB]; digits=2)) environmental benefit.\n")

# 	println(
# 		"With MILP allocation the auctineer achieves \
# 		$(round((data[!,:milp_allocation]'*data[!,:bids])/(data[!,:milp_allocation]'*data[!,:EB]); digits = 3)) €/EB, \
# 		with cost allocation \
# 		$(round((data[!,:bid_allocation]'*data[!,:bids])/(data[!,:bid_allocation]'*data[!,:EB]) ; digits=3)) €/EB, \
# 		and with environmental benefit allocation \
# 		$(round((data[!,:EB_allocation]'*data[!,:bids])/(data[!,:EB_allocation]'*data[!,:EB]) ; digits=3)) €/EB."
# 	)
# end

# ╔═╡ f95311ef-45df-4281-9bc5-b18e1b8fc94c
begin
	all_f = Figure(size=(1500,500))

	milp_axis = Axis(
		all_f[1,1], 
		title="Optimal allocation by MILP",
		xlabel="Bids",
		ylabel="Environmental benefit"
	)
	scatter!(
	...
	bid_axis = Axis(
		all_f[1,2], 
		title="Optimal allocation by lowest cost",
		xlabel="Bids",
		ylabel="Environmental benefit"
	)
	scatter!(
		bid_axis, 
		data[data[!,:bid_allocation], :bids],		
		data[data[!,:bid_allocation], :EB],
		label="Chosen",
		color=:green
	)
	scatter!(
	...
	eb_axis = Axis(
		all_f[1,3], 
		title="Optimal allocation by highest environmental benefit",
		xlabel="Bids",
		ylabel="Environmental benefit"
	)
	scatter!(
		eb_axis, 
		data[data[!,:EB_allocation], :bids],		
		data[data[!,:EB_allocation], :EB],
		label="Chosen",
		color=:green
	)
	scatter!(
...
manifest_format = "2.1"
project_hash = "68d4e7053b10d33423d2ad15c5f04bfeacc57acd"
